...
- P0-08 Temporal message handling is an opt-in macOS-only contract at `mise run contract:temporal-messages` (`NAGI_CONTRACT_TEMPORAL_MESSAGES=1`). Its separate wrapper requires a clean checked revision, then builds the feature-gated SDK test with the locked `rust@1.98.0` and `aqua:protocolbuffers/protobuf/protoc@36.1` tools into an exact dedicated target. The bounded build runs with private mode-0700 `HOME`, `CARGO_HOME`, and mise config/cache/state directories; it clone-copies only validated Cargo registry cache/index trees for an offline checksum-locked build, so developer Cargo configuration, credentials, and unpacked registry sources are not read. Before and after the build it captures exact `rustc`, `cargo`, and `protoc` probes and verifies the current-user native mise executable and unchanged SHA-256. Build, probe, and sidecar output stay in private bounded files, and the dedicated target is removed only after bounded cleanup. It runs the prebuilt binary through the P0-07 sidecar harness. `temporalio-client` 0.7.0 generates Signal-With-Start transport request IDs internally and does not expose one through `WorkflowStartOptions`, so resend safety is proved with a stable application logical message ID plus payload digest and an observed duplicate delivery. The SDK/protoc tools are build-only; no Temporal SDK code enters the standalone production binary or runtime, and no app bundle or signing profile is involved.
